...
public class JobDriver_BoardAcrossMaps : JobDriverAcrossMaps
{
  ...
  private static Toil BoardVehicle(Pawn pawnBoarding)
  {
    ...
    toil.initAction = delegate
    {
      ...
      {
        var vehicleAssigned =
          Patch_JobDriver_Board_MakeNewToils.GetAssignedSeat(lordJob_FormAndSendVehicles, pawnBoarding);
    ...
    if (ModsConfig.IdeologyActive)
    {
      switch (type)
      {
        case VehicleType.Sea:
          Find.HistoryEventsManager.RecordEvent(new HistoryEvent(HistoryEventDefOf_Vehicles.VF_BoardedSeaVehicle,
            pawn.Named(HistoryEventArgsNames.Doer)));
          return;
        case VehicleType.Air:
          Find.HistoryEventsManager.RecordEvent(new HistoryEvent(HistoryEventDefOf_Vehicles.VF_BoardedAirVehicle,
            pawn.Named(HistoryEventArgsNames.Doer)));
          return;
        case VehicleType.Land:
          Find.HistoryEventsManager.RecordEvent(new HistoryEvent(HistoryEventDefOf_Vehicles.VF_BoardedLandVehicle,
            pawn.Named(HistoryEventArgsNames.Doer)));
          return;
        case VehicleType.Universal:
          Find.HistoryEventsManager.RecordEvent(new HistoryEvent(HistoryEventDefOf_Vehicles.VF_BoardedUniversalVehicle,
            pawn.Named(HistoryEventArgsNames.Doer)));
          break;
        default:
          return;
