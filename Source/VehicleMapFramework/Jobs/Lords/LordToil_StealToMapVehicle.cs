...
{
  protected override DutyDef DutyDef => VMF_DefOf.VMF_Steal;
  ...
  public override void UpdateAllDuties()
  {
    ...
    for (var i = 0; i < lord.ownedPawns.Count; i++)
    {
      ...
      {
        if (!cover || (TryFindGoodOpportunisticTaskTarget(pawn, out thing, list) && !GenAI.InDangerousCombat(pawn)))
        {
  ...
  protected override bool TryFindGoodOpportunisticTaskTarget(Pawn pawn, out Thing target, List<Thing> alreadyTakenTargets)
  {
    ...
    var result = StealToMapVehiclesAIUtility.TryFindBestItemToSteal(pawn, JobGiver_Steal.ItemsSearchRadiusInitial,
      out var item, out _, alreadyTakenTargets);
    target = item;
    return result;
  }
}