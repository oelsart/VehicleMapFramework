...
public static class ReservationAcrossMapsUtility
{
  ...
  extension(Pawn claimant)
  {
    public bool CanReserve(LocalTargetInfo target, int maxPawns, int stackCount, ReservationLayerDef layer,
      bool ignoreOtherReservations, Map map)
    {
      if (claimant is null)
      {
      ...
      if (!claimant.Spawned || claimant.BaseMapOrCaravan != map.BaseMapOrCaravan ||
          !target.IsValid || target.ThingDestroyed ||
          target.HasThing && target.Thing.SpawnedOrAnyParentSpawned && target.Thing.MapHeld != map)
        return false;
      ...
      if (map.physicalInteractionReservationManager.IsReserved(target) &&
          !map.physicalInteractionReservationManager.IsReservedBy(claimant, target))
      {
        return false;
      }
      ...
      {
        if (claimant.Map.physicalInteractionReservationManager.IsReserved(target) &&
            !claimant.Map.physicalInteractionReservationManager.IsReservedBy(claimant, target))
        {
          return false;
        }
      }
      ...
      {
        if (reservation.Target == target && reservation.Layer == layer && reservation.Claimant == claimant &&
      ...
      if (target is { HasThing: true, Thing: Building building } && building.def.hasInteractionCell)
      {
        ...
        else if (map.reservationManager.TryGetReserver(interactionCell, claimant.Faction, out var pawn2) && pawn2.Spawned &&
                 pawn2 != claimant)
        {
          return false;
        }
    ...
    {
      return target.IsValid && !claimant.HasReserved(target, null, destMap) &&
             claimant.CanReserve(target, 1, -1, null, false, destMap);
    }

    public bool HasReserved(LocalTargetInfo target, Job job, Map destMap)
    {
      return claimant.Spawned && destMap.reservationManager.ReservedBy(target, claimant, job);
    }

    public bool Reserve(Map map, LocalTargetInfo target, Job job, int maxPawns = 1, int stackCount = -1,
      ReservationLayerDef layer = null, bool errorOnFailed = true, bool ignoreOtherReservations = false)
    {
      ...
      return map != null && map.reservationManager.Reserve(claimant, job, target, maxPawns, stackCount, layer, errorOnFailed,
        ignoreOtherReservations);
    }

    public int CanReserveStack(LocalTargetInfo target, int maxPawns, ReservationLayerDef layer,
      bool ignoreOtherReservations, Map map)
    {
      if (claimant == null)
      {
      ...
      if (!claimant.Spawned || claimant.BaseMapOrCaravan != map.BaseMapOrCaravan || !target.IsValid || target.ThingDestroyed ||
          target.HasThing && target.Thing.SpawnedOrAnyParentSpawned && target.Thing.MapHeld != map)
        return 0;
      var num1 = target.HasThing ? target.Thing.stackCount : 1;
      var num2 = 0;
      if (!ignoreOtherReservations)
      {
        ...
        for (var index = 0; index < tmpReservations.Count; ++index)
        {
          var reservation = tmpReservations[index];
          if (!(reservation.Target != target) && reservation.Layer == layer && reservation.Claimant != claimant &&
              RespectsReservationsOf(claimant, reservation.Claimant))
          {
            ...
            if (num3 >= maxPawns || num2 >= num1)
              return 0;
          }
        }
      }
      return Mathf.Max(num1 - num2, 0);
    }
  }
