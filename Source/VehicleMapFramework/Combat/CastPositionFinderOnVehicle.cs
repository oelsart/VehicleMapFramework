...
public static class CastPositionFinderOnVehicle
{
  public static bool TryFindCastPosition(CastPositionRequest newReq, out IntVec3 dest)
  {
    ...
    var cellRect = req.caster.IsOnVehicleMapOf(out var vehicle) ? vehicle.MapRect : CellRect.WholeMap(req.caster.Map);
  ...
  private static void EvaluateCell(IntVec3 c)
  {
    ...
    if (maxRangeFromTargetSquared is > 0.01f and < 250000f &&
        (c - targetLoc).LengthHorizontalSquared > maxRangeFromTargetSquared)
    {
    ...
    if (!CrossMapReachabilityUtility.CanReach(casterMap, req.caster.Position, c, PathEndMode.OnCell,
          TraverseParms.For(req.caster, Danger.Some), req.target.Map) &&
        !req.caster.IsOnVehicleMap && !req.target.IsOnVehicleMap)
    {
      if (DebugViewSettings.drawCastPositionSearch)
      {
