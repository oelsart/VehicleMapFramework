...
public static class AttackTargetFinderOnVehicle
{
  ...
  public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags,
    ...
    float maxTravelRadiusFromLocus = 3.4028235E+38f, bool canBashDoors = false,
    bool canTakeTargetsCloserThanEffectiveMinRange = true, bool canBashFences = false, bool onlyRanged = false)
  {
    ...
    {
      losValidator = vec3 =>
        !vec3.InBounds(searcherThing.BaseMap()) || !vec3.AnyGas(searcherThing.BaseMap(), GasType.BlindSmoke);
    }
    ...
    Predicate<IAttackTarget> innerValidator = delegate(IAttackTarget t)
    {
      ...
      if (minDistSquared > 0f &&
          (searcherThing.PositionOnBaseMapSpawned - thing.PositionOnBaseMapSpawned).LengthHorizontalSquared <
          minDistSquared)
      {
        return false;
      }
      ...
      if (maxTravelRadiusFromLocus < 9999f &&
          (thing.PositionOnBaseMapSpawned - locus).LengthHorizontalSquared > maxLocusDistSquared)
      {
        return false;
      }
      ...
      {
        var roof = thing.PositionOnBaseMapSpawned.GetRoof(baseMap);
      ...
      {
        if (losValidator != null && (!losValidator(searcherThing.PositionOnBaseMapSpawned) ||
                                     !losValidator(thing.PositionOnBaseMapSpawned)))
        {
          return false;
        }
      ...
      {
        if (thing.PositionOnBaseMapSpawned.Fogged(baseMap))
        {
          return false;
        }
      }
    ...
    {
      tmpTargets.Clear();
      tmpTargets.AddRange(searcherThing.Map.BaseMapAndVehicleMaps(false)
        .SelectMany(m => m.attackTargetsCache.GetPotentialTargetsFor(searcher)));
      validTargets.Clear();
      for (var i = 0; i < tmpTargets.Count; i++)
      {
        var attackTarget = tmpTargets[i];
        if (attackTarget.Thing.PositionOnBaseMapSpawned.InHorDistOf(searcherThing.PositionOnBaseMapSpawned, maxDist) &&
            innerValidator(attackTarget))
        {
      ...
      if (targetToHit != null || searcher is Building_Turret ||
          (searcher is Pawn && searcherPawn.CurJobDef == JobDefOf.ManTurret))
      {
        return targetToHit;
      }
           ...
           (flags & TargetScanFlags.NeedReachable) != TargetScanFlags.None))
      {
        return (IAttackTarget)GenClosestCrossMap.ClosestThing_Global(
          searcher.Thing.PositionOnBaseMapSpawned,
          validTargets,
          maxDist,
      ...
      return (IAttackTarget)GenClosestCrossMap.ClosestThing_Global(searcher.Thing.PositionOnBaseMapSpawned,
        validTargets, maxDist);
    }

    if (searcherPawn?.mindState.duty is { radius: > 0f } && !searcherPawn.InMentalState)
    {
      var oldValidator = innerValidator;
      innerValidator = t => oldValidator(t) && t.Thing.PositionOnBaseMapSpawned
    ...
    var attackTarget2 = (IAttackTarget)GenClosestCrossMap.ClosestThingReachable(searcherThing.Position,
      searcherThing.Map, ThingRequest.ForGroup(ThingRequestGroup.AttackTarget), PathEndMode.Touch,
      TraverseParms.For(searcherPawn, Danger.Deadly, TraverseMode.ByPawn, canBashDoors, false, canBashFences), maxDist,
  ...
  {
    return t is Pawn pawn && !pawn.IsCombatant() &&
           ((flags & TargetScanFlags.IgnoreNonCombatants) != TargetScanFlags.None ||
            !GenSightOnVehicle.LineOfSightThingToThing(searcherThing, pawn));
  }
  ...
  private static bool CanReach(Thing searcher, Thing target, bool canBashDoors, bool canBashFences)
  {
    ...
    else
    {
      ...
      if (!CrossMapReachabilityUtility.CanReach(searcher.Map, searcher.Position, target, PathEndMode.Touch,
            TraverseParms.For(mode), target.Map))
      {
        return false;
      }
    }
  ...
  private static IAttackTarget FindBestReachableMeleeTarget(Predicate<IAttackTarget> validator, Pawn searcherPawn,
    float maxTargDist, bool canBashDoors, bool canBashFences)
  {
  ...
  {
    return verb != null && verb.CanHitTargetFrom(searcher.Thing.PositionOnBaseMapSpawned, target.Thing);
  }
    ...
    IAttackTargetSearcher searcher, Verb verb)
  {
    return GetAvailableShootingTargetsByScore(targets, searcher, verb)
      .TryRandomElementByWeight(x => x.Second, out var pair) ? pair.First : null;
  }
  ...
  {
    var num = 60f;
    num -= Mathf.Min((target.Thing.PositionOnBaseMapSpawned - searcher.Thing.PositionOnBaseMapSpawned).LengthHorizontal,
      40f);
    ...
    num -= CoverUtility.CalculateOverallBlockChance(target.Thing.Position,
      searcher.Thing.PositionOnAnotherThingMap(target.Thing), target.Thing.Map) * 10f;
    if (target is Pawn pawn)
    {
      num -= NonCombatantScore(pawn);
      if (verb.verbProps.ai_TargetHasRangedAttackScoreOffset != 0f && pawn.CurrentEffectiveVerb != null &&
          pawn.CurrentEffectiveVerb.verbProps.Ranged)
      {
    ...
    Map map, Verb verb)
  {
    ...
    var position = target.Thing.Map == map
  ...
  public static void DebugDrawAttackTargetScores_Update()
  {
    ...
    var availableShootingTargetsByScore =
      GetAvailableShootingTargetsByScore(tmpTargets, attackTargetSearcher, currentEffectiveVerb);
    for (var j = 0; j < availableShootingTargetsByScore.Count; j++)
    {
      GenDraw.DrawLineBetween(attackTargetSearcher.Thing.DrawPos,
        availableShootingTargetsByScore[j].First.Thing.DrawPos);
    }
  }
  ...
  public static void DebugDrawAttackTargetScores_OnGUI()
  {
    ...
    for (var i = 0; i < list.Count; i++)
    {
      ...
      if (thing != attackTargetSearcher)
      {
        ...
        {
          text = GetShootingTargetScore((IAttackTarget)thing, attackTargetSearcher, currentEffectiveVerb)
            .ToString("F0");
  ...
  public static IAttackTarget CompareTarget(IAttackTarget target1, IAttackTarget target2,
