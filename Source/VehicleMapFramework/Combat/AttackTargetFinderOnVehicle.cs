...
public static class AttackTargetFinderOnVehicle
{
  ...
  public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags, Predicate<Thing> validator = null, float minDist = 0f, float maxDist = 9999f, IntVec3 locus = default, float maxTravelRadiusFromLocus = 3.4028235E+38f,
    bool canBashDoors = false, bool canTakeTargetsCloserThanEffectiveMinRange = true, bool canBashFences = false, bool onlyRanged = false)
  {
    ...
    Predicate<IAttackTarget> innerValidator = delegate(IAttackTarget t)
    {
      ...
      if (!canTakeTargetsCloserThanEffectiveMinRange)
      {
        ...
        if (num2 > 0f && (searcherThing.PositionOnBaseMapSpawned - thing.PositionOnBaseMapSpawned).LengthHorizontalSquared < num2 * num2)
        {
          return false;
        }
      }
      if (maxTravelRadiusFromLocus < 9999f && (thing.PositionOnBaseMapSpawned - locus).LengthHorizontalSquared > maxLocusDistSquared)
      {
        return false;
      }
    ...
    if ((HasRangedAttack(searcher) || onlyRanged || hasGrappling) && searcherPawn is not { InAggroMentalState: true })
    {
      ...
      for (var i = 0; i < tmpTargets.Count; i++)
      {
        ...
        if (attackTarget.Thing.PositionOnBaseMapSpawned.InHorDistOf(searcherThing.PositionOnBaseMapSpawned, maxDist) && innerValidator(attackTarget))
        {
      ...
      return (IAttackTarget)GenClosestCrossMap.ClosestThing_Global(searcher.Thing.PositionOnBaseMapSpawned, validTargets, maxDist);
    }
    ...
    var attackTarget2 = (IAttackTarget)GenClosestCrossMap.ClosestThingReachable(searcherThing.Position,
      ...
      maxDist,
      x => innerValidator((IAttackTarget)x),
      null,
      0,
      maxDist > 800f ? -1 : 40);
  ...
  private static IAttackTarget FindBestReachableMeleeTarget(Predicate<IAttackTarget> validator, Pawn searcherPawn, float maxTargDist, bool canBashDoors, bool canBashFences)
  {
    ...
    IAttackTarget bestTargetOnCell(IntVec3 x)
    {
      ...
      for (var i = 0; i < thingList.Count; i++)
      {
        ...
        if (thing is IAttackTarget attackTarget && validator(attackTarget) && ReachabilityImmediate.CanReachImmediate(x, thing, searcherPawn.Map, PathEndMode.Touch, searcherPawn) &&
            (searcherPawn.CanReachImmediate(thing, PathEndMode.Touch) || searcherPawn.Map.attackTargetReservationManager.CanReserve(searcherPawn, attackTarget)))
        {
          return attackTarget;
        }
      }
  ...
  {
    var num = 60f;
    num -= Mathf.Min((target.Thing.PositionOnBaseMapSpawned - searcher.Thing.PositionOnBaseMapSpawned).LengthHorizontal, 40f);
