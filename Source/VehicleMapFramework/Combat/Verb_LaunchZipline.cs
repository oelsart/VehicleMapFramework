  ...
  protected override bool TryCastShot()
  {
    ...
    if (verbProps.ForcedMissRadius > 0.5f)
    {
      ...
      var num2 = VerbUtility.CalculateAdjustedForcedMiss(num,
        target.Cell.ToBaseMapCoord(destMap) - caster.PositionOnBaseMap);
      if (num2 > 0.5f)
      {
        ...
        if (forcedMissTarget != target.Cell)
        {
          ...
          projectile2.Launch(manningPawn, drawPos, forcedMissTarget, target, projectileHitFlags, preventFriendlyFire,
            equipmentSource);
          return true;
        }
      }
    ...
    if (verbProps.canGoWild && !Rand.Chance(shotReport.AimOnTargetChance_IgnoringPosture))
    {
      ...
      projectile2.Launch(manningPawn, drawPos, resultingLine.Dest, target, projectileHitFlags2, preventFriendlyFire,
    ...
    if (target.Thing != null && target.Thing.def.CanBenefitFromCover && !Rand.Chance(shotReport.PassCoverChance))
    {
      ...
      projectile2.Launch(manningPawn, drawPos, randomCoverToMissInto, target, projectileHitFlags3, preventFriendlyFire,
        equipmentSource, targetCoverDef);
      return true;
    }
