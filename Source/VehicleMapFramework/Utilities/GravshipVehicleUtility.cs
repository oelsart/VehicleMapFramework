...
namespace VehicleMapFramework
{
  public static class GravshipVehicleUtility
  {
    ...
    private static readonly Func<IntVec3, Map, AcceptanceReport> IsValidCell =
      (Func<IntVec3, Map, AcceptanceReport>)AccessTools.Method(typeof(Designator_MoveGravship), "IsValidCell")
        .CreateDelegate(typeof(Func<IntVec3, Map, AcceptanceReport>));
    ...
    private static readonly Func<WorldComponent_GravshipController, Building_GravEngine, Gravship>
      RemoveGravshipFromMap =
        (Func<WorldComponent_GravshipController, Building_GravEngine, Gravship>)AccessTools
          .Method(typeof(WorldComponent_GravshipController), "RemoveGravshipFromMap")
          .CreateDelegate(typeof(Func<WorldComponent_GravshipController, Building_GravEngine, Gravship>));

    public static readonly Func<WorldComponent_GravshipController, Gravship, IntVec3, Map, Building_GravEngine>
      PlaceGravship =
        (Func<WorldComponent_GravshipController, Gravship, IntVec3, Map, Building_GravEngine>)AccessTools
          .Method(typeof(WorldComponent_GravshipController), "PlaceGravship")
          .CreateDelegate(typeof(Func<WorldComponent_GravshipController, Gravship, IntVec3, Map, Building_GravEngine>));

    public static bool GravshipProcessInProgress => GravshipUtility.generatingGravship ||
                                                    GravshipPlacementUtility.placingGravship || placingGravshipVehicle;

    public static void PlaceGravshipVehicleUnSpawned(Building_GravEngine engine, IntVec3 loc, Rot4 rot,
      VehiclePawnWithMap vehicle, bool forced = false)
    {
    ...
    public static AcceptanceReport PlaceGravshipVehicle(Building_GravEngine engine, VehiclePawnWithMap vehicle,
      bool forced = false)
    {
    ...
    public static AcceptanceReport GenerateGravshipVehicle(Building_GravEngine engine, VehicleDef baseDef,
      bool checkStability = true)
    {
      ...
      Thing thing = null;
      if (cells.Any(c =>
            c.GetThingList(map).Any(t =>
              (thing = t).def.PlaceWorkers?.Any(p => p is PlaceWorker_ForbidOnVehicle) ?? false)))
      {
      ...
      try
      {
        var vehicleDef = GenerateGravshipVehicleDef(props);
        ...
        {
          var transform = new TransformData(vehiclePawn.DrawPos, vehiclePawn.FullRotation,
            vehiclePawn.Transform.rotation);
        ...
        {
          PlaceGravship(null, state.gravship, state.minOffset.RotatedBy(state.rotCounter) + IntVec3.NorthEast,
            state.vehiclePawn.VehicleMap);
    ...
    public static VehicleDef GenerateGravshipVehicleDef(VehicleMapProps_Gravship props, UniqueVehicleManager manager = null)
    {
      if (!ModsConfig.OdysseyActive) return null;

      var component = manager ?? Current.Game.GetComponent<UniqueVehicleManager>();
      var vehicleDef = component.ClaimUniqueVehicleDef(props.baseDef);
      if (!props.defName.NullOrEmpty())
        ScribeMetaHeaderUtility.modListChanged = true;
      vehicleDef.label = "Gravship".Translate();
      vehicleDef.size = props.size;
      vehicleDef.graphicData = new GraphicDataRGB();
      vehicleDef.graphicData.CopyFrom(props.baseDef.graphicData);
      vehicleDef.graphicData.texPath = "VehicleMapFramework/ClearTex";
      vehicleDef.graphicData.drawSize = props.size.ToVector2();
      vehicleDef.modContentPack = VehicleMapFramework.mod.Content;
    ...
    public static AcceptanceReport CheckGravshipVehicleStability(Building_GravEngine engine, Rot4 rot,
