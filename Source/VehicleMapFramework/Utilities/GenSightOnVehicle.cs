...
public static class GenSightOnVehicle
{
  public static bool LineOfSight(IntVec3 start, IntVec3 end, Map map, bool skipFirstCell, Func<IntVec3, bool> validator = null, int halfXOffset = 0, int halfZOffset = 0)
  {
    ...
    if (map.IsVehicleMapOf(out var vehicle))
    {
      ...
      {
        return LineOfSightVehicleToVehicle(start, end, map, skipFirstCell, validator, halfXOffset, halfZOffset);
      }
    }
  ...
  {
    return LineOfSight(thing.PositionOnBaseMapSpawned, target.CellOnBaseMapSpawned(), thing.BaseMap(), skipFirstCell, validator);
  }
  ...
  {
    return LineOfSight(start.PositionOnBaseMapSpawned, end.PositionOnBaseMapSpawned, start.BaseMap(), skipFirstCell, validator);
  }
  ...
  public static bool LineOfSight(IntVec3 start, IntVec3 end, Map map, CellRect startRect, CellRect endRect, Func<IntVec3, bool> validator = null)
  {
    if (map.IsVehicleMapOf(out var vehicle))
    {
      ...
      {
        return LineOfSightVehicleToVehicle(start, end, map, false, validator);
      }
    }
    if (!start.InBounds(map) || !end.InBounds(map)) return false;
