...
public class VehicleCaravanIncidentUtility
{
  ...
  public static Map SetupCaravanAttackMap(VehicleCaravan caravan, List<VehiclePawnWithMap> vehicles, List<Pawn> enemies, bool sendLetterIfRelatedPawns, WorldObjectDef mapParent)
  {
    try
    {
      ...
      // キャラバンスポーン
      EnterMapUtilityVehicles.EnterMap(caravan,
        map,
  ...
  public static void SpawnEnemies(Map map, List<VehiclePawnWithMap> vehicles, List<Pawn> enemies, Rot4? edge = null, VehiclePawn playerVehicle = null)
  {
    ...
    for (var i = 0; i < vehicles.Count; i++)
    {
      ...
      if (vehicle.CompNpcVehicleMap != null)
      {
        ...
        PrefabUtility.SpawnPrefab(prefab,
          vehicleMap,
          vehicleMap.Center,
          Rot4.North,
          vehicle.Faction,
      ...
      LongEventHandler.ExecuteWhenFinished(() =>
      {
        ...
        for (var j = index; index < j + count; index++)
        {
          ...
          var pos = CellFinderExtended.RandomSpawnCellForPawnNear(vehicleMap.Center,
            vehicleMap,
            pawn,
            _ => true);
    ...
    bool SpawnVehicle(VehiclePawnWithMap vehicle)
    {
      ...
      var pos2 = CellFinderExtended.RandomSpawnCellForPawnNear(cell,
        map,
        vehicle,
