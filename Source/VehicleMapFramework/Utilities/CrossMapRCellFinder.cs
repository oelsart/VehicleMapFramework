...
{
  public static IntVec3 BestOrderedGotoDestNear(IntVec3 root, Pawn searcher, Predicate<IntVec3> cellValidator,
    bool reachable, Map map)
  {
    if (map is null)
    {
  ...
  public static IntVec3 GoodDestNearFromTo(IntVec3 from, IntVec3 to, Pawn searcher, Map map,
    Predicate<IntVec3> cellValidator = null, bool reachable = true, bool reserve = true, float radius = 30f)
  {
    if (map is null)
      return IntVec3.Invalid;
    if (IsGoodDest(to))
  ...
  private static bool IsGoodDestinationFor(IntVec3 c, Pawn pawn, Map map, bool careAboutDanger)
  {
    ...
    return !c.IsForbidden(pawn) && (!careAboutDanger || c.GetDangerFor(pawn, map) != Danger.Deadly) &&
           (!careAboutDanger || !PawnUtility.KnownDangerAt(c, map, pawn)) && (!careAboutDanger || !VacuumConcernTo(c));
  ...
  public static bool TryFindGoodAdjacentSpotToTouch(Pawn toucher, Thing touchee, out IntVec3 result)
  {
    ...
    {
      if (IsGoodDestinationFor(item, toucher, map, careAboutDanger: true) &&
          toucher.CanReach(item, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn, map) &&
          ReachabilityImmediate.CanReachImmediate(item, touchee, toucher.Map, PathEndMode.Touch, toucher))
      {
        ...
        if (num2 < num || (intVec.GetTerrain(map).avoidWander && !item.GetTerrain(map).avoidWander) ||
            (intVec.GetFirstThing<Building_Trap>(map) != null && item.GetFirstThing<Building_Trap>(map) == null))
        {
          num = num2;
          intVec = item;
        }
