...
public static class GenClosestCrossMap
{
  ...
  private static bool EarlyOutSearch(IntVec3 start, ref Map map, ThingRequest thingReq,
  ...
  public static Thing ClosestThingReachable(IntVec3 root, Map map, ThingRequest thingReq, PathEndMode peMode,
    ...
    bool ignoreEntirelyForbiddenRegions = false, bool lookInHaulSources = false)
  {
    ...
    {
      Log.ErrorOnce(
        "ClosestThingReachable with thing request group " + thingReq.group +
    ...
    if (thingReq is { IsUndefined: false, CanBeFoundInRegion: true })
    {
      ...
      thing = RegionwiseBFSWorker(root, map, thingReq, peMode, traverseParams, validator, null, searchRegionsMin, num,
        maxDistance, out var num2, traversableRegionTypes, ignoreEntirelyForbiddenRegions, lookInHaulSources);
  ...
  public static Thing ClosestThing_Regionwise_ReachablePrioritized(IntVec3 root, Map map, ThingRequest thingReq,
    PathEndMode peMode, TraverseParms traverseParams, float maxDistance = 9999f, Predicate<Thing> validator = null,
    Func<Thing, float> priorityGetter = null, int minRegions = 24, int maxRegions = 30, bool lookInHaulSources = false)
  {
    if (thingReq is { IsUndefined: false, CanBeFoundInRegion: false })
    {
      Log.ErrorOnce(
        "ClosestThing_Regionwise_ReachablePrioritized with thing request group " + thingReq.group +
        ". This will never find anything because this group is never stored in regions. Most likely a global search should have been used.",
        738476712);
      return null;
    }
    ...
    {
      result = RegionwiseBFSWorker(root, map, thingReq, peMode, traverseParams, validator, priorityGetter, minRegions,
        maxRegions, maxDistance, out var _, RegionType.Set_Passable, ignoreEntirelyForbiddenRegions: false,
        lookInHaulSources);
    }
  ...
  public static Thing RegionwiseBFSWorker(IntVec3 root, Map map, ThingRequest req, PathEndMode peMode,
    TraverseParms traverseParams, Predicate<Thing> validator, Func<Thing, float> priorityGetter, int minRegions,
    int maxRegions, float maxDistance, out int regionsSeen, RegionType traversableRegionTypes = RegionType.Set_Passable,
    bool ignoreEntirelyForbiddenRegions = false, bool lookInHaulSources = false)
  {
    regionsSeen = 0;
    switch (traverseParams.mode)
    {
      ...
      case TraverseMode.PassAllDestroyablePlayerOwnedThings:
        Log.Error(
          "RegionwiseBFSWorker with traverseParams.mode PassAllDestroyablePlayerOwnedThings. Use ClosestThingGlobal.");
        return null;
    ...
    {
      Log.ErrorOnce(
        string.Concat("RegionwiseBFSWorker with thing request group ", req.group,
          ". This group is never stored in regions. Most likely a global search should have been used."), 385766189);
      return null;
    }
    ...
    var basePos = map.IsVehicleMapOf(out var vehicle) ? root.ToBaseMapCoord(vehicle) : root;
    regionProcessorClosestThingReachable.SetParameters(traverseParams, maxDistance, basePos,
      ignoreEntirelyForbiddenRegions, req, peMode, priorityGetter, validator, minRegions, 9999999f, 0, float.MinValue,
      null, lookInHaulSources, map);
    RegionTraverserAcrossMaps.BreadthFirstTraverse(region, regionProcessorClosestThingReachable, maxRegions,
  ...
  public static Thing ClosestThing_Global(IntVec3 centerOnBaseMap, IEnumerable searchSet, float maxDistance = 99999f,
    Predicate<Thing> validator = null, Func<Thing, float> priorityGetter = null, bool lookInHaulSources = false)
  {
    if (searchSet == null)
    {
      return null;
  ...
  public static Thing ClosestThing_Global_Reachable(IntVec3 center, Map map, IEnumerable<Thing> searchSet,
    PathEndMode peMode, TraverseParms traverseParams, float maxDistance = 9999f, Predicate<Thing> validator = null,
    Func<Thing, float> priorityGetter = null, bool canLookInHaulableSources = false)
  {
    if (searchSet == null)
    {
      return null;
    ...
    void ValidateThing(Thing t, float distSquared)
    {
      if (!CrossMapReachabilityUtility.CanReach(departMap, center, t.SpawnedParentOrMe, peMode, traverseParams,
            t.MapHeld))
        return;
