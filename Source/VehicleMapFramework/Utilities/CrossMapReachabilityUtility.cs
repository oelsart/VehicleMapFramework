...
public static class CrossMapReachabilityUtility
{
  ...
  private static readonly AStar<MapTraverse> aStar = new(Traverser.Cost,
    traverser.Neighbors,
    traverser.FinalCheck,
    traverser.CanEnter,
    traverser.Heuristic,
    Traverser.ProcessPath,
    traverser.DebugDrawEnterNode);
            ...
            bool CanReachBasic(out TargetInfo exitSpot, out TargetInfo enterSpot)
            {
              ...
              bool CanReach2(IntVec3 cell, IntVec3 cell2, IntVec3 cell3, IntVec3 cell4)
              {
                return CellCheck(cell2, departBaseMap, pawn) &&
                       ...
                       departMap.reachability.CanReach(root,
                         cell,
                         PathEndMode.OnCell,
                         traverseParms) &&
                       departBaseMap.reachability.CanReach(cell2,
                         cell3,
                         PathEndMode.OnCell,
                         traverseParms2) &&
                  ...
                  foreach (var targetMap in candidateMaps)
                  {
                    if (!visitedMaps.Contains(targetMap) &&
                        ability.TryFindCastPositionFromTo(
                          new TargetInfo(start, map),
                          new TargetInfo(targetMap.Center, targetMap),
                          out var castSpot,
                          out var targSpot))
                    {
  ...
  extension(Pawn pawn)
  {
    ...
    {
      var traverseParms = TraverseParms.For(pawn, maxDanger, mode, canBashDoors, canBashFences: canBashFences);
  ...
  private class Traverser
  {
    ...
    public IEnumerable<MapTraverse> Neighbors(MapTraverse current)
    {
      ...
      if (_ability is not null)
      {
        ...
        for (var i = 0; i < _tmpCandidates.Count; i++)
        {
          ...
          else if (_ability.TryFindCastPositionFromTo(current.enterSpot,
                     new TargetInfo(_destBaseMapCoord, map2),
                     out var castSpot,
                     out var targSpot))
          {
            yield return new MapTraverse(castSpot, targSpot, false);
    ...
    public bool CanEnter(MapTraverse from, MapTraverse to)
    {
      return CellCheck(to.exitSpot.Cell, to.exitSpot.Map, _traverseParms.pawn) &&
             ...
             from.enterSpot.Map.reachability.CanReach(from.enterSpot.Cell,
               to.exitSpot.Cell,
               PathEndMode.OnCell,
               _traverseParms2) &&
    ...
    public bool FinalCheck(MapTraverse from, MapTraverse to)
    {
      ...
      {
        if (from.enterSpot.Map.reachability.CanReach(from.enterSpot.Cell,
              region.AnyCell,
              PathEndMode.OnCell,
              _traverseParms2))
        {
    ...
    Action<T> debugAction = null) where T : IEquatable<T>
  {
    ...
    private bool CreateNode(T current, T neighbor, out Node node)
    {
      ...
      node = new Node
      {
        parent = current, cost = cost(current, neighbor), heuristic = heuristic(neighbor)
      };
      return true;
    }
