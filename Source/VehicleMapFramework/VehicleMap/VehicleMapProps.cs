...
public class VehicleMapProps : DefModExtension
{
  ...
  public IEnumerable<IntVec2> FilledStructureCells => filledStructureCells
    .Union(filledStructureCellRects.SelectMany(r => r.Cells2D)).Select(c => c + IntVec2.One);

  public IEnumerable<IntVec2> EmptyStructureCells => emptyStructureCells
    .Union(emptyStructureCellRects.SelectMany(r => r.Cells2D)).Select(c => c + IntVec2.One);

  public IEnumerable<IntVec2> ExpandableCells => expandableCells.Union(expandableCellRects.SelectMany(r => r.Cells2D))
    .Select(c => c + IntVec2.One);

  public IEnumerable<IntVec2> OutOfBoundsCells
  {
    get
    {
      return new CellRect(0, 0, size.x + 2, size.z + 2).EdgeCells.Select(c => c.ToIntVec2).Union(outOfBoundsCells
        .Union(outOfBoundsCellRects.SelectMany(r => r.Cells2D)).Select(c => c + IntVec2.One));
    }
  }
  ...
  public EdgeSpace EdgeSpaceByRot(Rot8 rot)
  {
    switch (rot.AsInt)
    {
      ...
      case Rot8.NorthEastInt:
        return edgeSpaceNorthEast ?? (edgeSpaceNorthEast = edgeSpaceNorth?.ToDiagonal() ??
                                                           (edgeSpaceNorth = edgeSpaceSouth?.FlipVertical() ??
                                                                             edgeSpace).Value.ToDiagonal()).Value;
      case Rot8.SouthEastInt:
        return edgeSpaceSouthEast ?? (edgeSpaceSouthEast = edgeSpaceSouth?.ToDiagonal() ??
                                                           (edgeSpaceSouth = edgeSpaceNorth?.FlipVertical() ??
                                                                             edgeSpace).Value.ToDiagonal()).Value;
      case Rot8.SouthWestInt:
        return edgeSpaceSouthWest ?? (edgeSpaceSouthWest = edgeSpaceSouth?.ToDiagonal() ??
                                                           (edgeSpaceSouth = edgeSpaceNorth?.FlipVertical() ??
                                                                             edgeSpace).Value.ToDiagonal()).Value;
      case Rot8.NorthWestInt:
        return edgeSpaceNorthWest ?? (edgeSpaceNorthWest = edgeSpaceNorth?.ToDiagonal() ??
                                                           (edgeSpaceNorth = edgeSpaceSouth?.FlipVertical() ??
                                                                             edgeSpace).Value.ToDiagonal()).Value;
  ...
  public struct EdgeSpace
  {
    ...
    {
      return this with { east = west, west = east };
    }

    public EdgeSpace FlipVertical()
    {
      return this with { north = south, south = north };
    }
