...
public class SectionLayer_LightingOnVehicle : SectionLayer
{
  ...
  public override bool Visible => DebugViewSettings.drawLightingOverlay &&
                                  (Find.CurrentMap != Map || VehicleMapFramework.settings.drawPlanet);
  ...
  public override void Regenerate()
  {
    ...
    for (var l = origRect.minZ; l <= maxZ; l++)
    {
      ...
      while (m <= maxX)
      {
        ...
        array[num13] = new Color32((byte)(colorInt.r / 4), (byte)(colorInt.g / 4), (byte)(colorInt.b / 4),
          (byte)(colorInt.a / 4));
        m++;
        num12++;
        num13++;
    ...
    if (expand.Any(e => e))
    {
      for (var i = 0; i < ExpandSize; i++)
      {
        ...
        for (var j = 0; j < 4; j++)
        {
          ...
          if (expand[j])
          {
            var edgeRect = rect.GetEdgeRect(rot);
            ...
            foreach (var cell in edgeRect)
            {
              ...
              var max = Mathf.Max(edgeColorCorner.r, Mathf.Max(edgeColorCorner.g, edgeColorCorner.b));
              var factor = Mathf.Max(max - 255f / ExpandSize * length, 0f) / max;
              array[corner] = new Color32((byte)(edgeColorCorner.r * factor), (byte)(edgeColorCorner.g * factor),
                (byte)(edgeColorCorner.b * factor), (byte)(edgeColorCorner.a * factor));
            }

            var edgeRect2 = rect2.GetEdgeRect(rot);
            ...
            foreach (var cell in edgeRect2)
            {
              ...
              array[center] = new Color32((byte)(colorInt.r / 4), (byte)(colorInt.g / 4), (byte)(colorInt.b / 4),
                (byte)(colorInt.a / 4));
            }
          }
        }
  ...
  private void CalculateVertexIndices(int worldX, int worldZ, out int botLeft, out int topLeft, out int topRight,
    out int botRight, out int center)
  {
