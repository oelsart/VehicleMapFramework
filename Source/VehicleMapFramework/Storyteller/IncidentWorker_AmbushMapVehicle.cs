...
public abstract class IncidentWorker_AmbushMapVehicle : IncidentWorker
{
  ...
  protected override bool CanFireNowSub(IncidentParms parms)
  {
    return parms.target is Map map
      ? CellFinder.TryFindRandomEdgeCellWith(x => x.Standable(map) && map.reachability.CanReachColony(x), map,
        CellFinder.EdgeRoadChance_Hostile, out _)
      : parms.target is VehicleCaravan &&
  ...
  protected override bool TryExecuteWorker(IncidentParms parms)
  {
        ...
        !TryFindCellEdgeCell(Rot4.East) && !TryFindCellEdgeCell(Rot4.West))
    {
      ...
      VMF_Log.DebugWarning($"{this.GetType()}: could not find edge cell");
      return false;
    }
    ...
    if (map != null)
    {
      ...
      {
        CleanUpGeneratedVehicles(generatedVehicles);
        VMF_Log.DebugWarning($"{this.GetType()} fail to execute");
        return false;
      }
    ...
    bool TryFindCellEdgeCell(Rot4 rot) =>
      CellFinderExtended.TryFindRandomEdgeCellWith(c => largestVehicle.CellRectStandable(map, c),
        map, rot, largestVehicle.VehicleDef, CellFinder.EdgeRoadChance_Hostile, out existingMapEdgeCell);
  }

  private bool DoExecute(IncidentParms parms, List<VehiclePawnWithMap> generatedVehicles, List<Pawn> generatedEnemies,
    IntVec3 existingMapEdgeCell)
  {
    ...
    {
      map = VehicleCaravanIncidentUtility.SetupCaravanAttackMap(vehicleCaravan, generatedVehicles, generatedEnemies,
        false, MapParentDef);
      flag = true;
    }
    else return false;
    ...
    PawnRelationUtility.Notify_PawnsSeenByPlayer_Letter(generatedEnemies, ref taggedString, ref taggedString2,
      GetRelatedPawnsInfoLetterText(parms), true);
    SendStandardLetter(taggedString, taggedString2, GetLetterDef(generatedEnemies[0], parms), parms,
      generatedEnemies[0]);
    if (flag)
    {
