...
public class VehiclePawnWithMap : VehiclePawn, IEventManager<MapVehicleEventDef>
{
  ...
  {
    return VehicleMap.mapPawns.FreeHumanlikesSpawnedOfFaction(Faction).Empty() && base.ThreatDisabled(disabledFor);
  }
  ...
  public override IEnumerable<Gizmo> GetGizmos()
  {
    ...
    if (DebugSettings.ShowDevGizmos)
    {
      ...
      yield return new Command_Toggle
      {
        defaultLabel = "Debug draw: bridge cells", Order = 5005, isActive = () => CompMapExpander.debugDraw, toggleAction = () => CompMapExpander.debugDraw = !CompMapExpander.debugDraw
      };
      yield return new Command_Action
      {
        ...
        action = () =>
        {
          ...
          {
            interiorMap.debugDrawer.FlashCell(c);
          }
        }
      };
      yield return new Command_Action
      {
        ...
        action = () =>
        {
          foreach (var c in CachedWalkableMapEdgeCells.Keys)
          {
            interiorMap.debugDrawer.FlashCell(c);
          }
        }
  ...
  public override void Destroy(DestroyMode mode = DestroyMode.Vanish)
  {
    ...
    if (map is not null)
    {
      ...
      for (var i = allThings.Count - 1; i >= 0; i--)
      {
        ...
        if (mode != DestroyMode.Vanish && thing is { Destroyed: false })
        {
          ...
          else if (thing is not (Explosion or Projectile))
          {
            ...
            {
              CellFinder.TryFindRandomCellNear(positionOnBaseMap,
                map,
                50,
                c => GenPlace.TryPlaceThing(thing, c, Map, ThingPlaceMode.Near),
                out _);
            }
          }
        }
      ...
      if (flag)
      {
        ...
        Find.LetterStack.ReceiveLetter("VF_BoatSunk".Translate(),
          text,
  ...
  public override void DrawAt(in Vector3 drawLoc, Rot8 rot, float rotation)
  {
    if (!Spawned)
    {
      interiorMap?.GetDetachedMapComponent<VehiclePositionManager>().AllClaimants.DoIf(v => v.def.graphicData?.drawRotated ?? false, v => { v.Transform.rotation = rotation.FlipAngle(v); });
  ...
  public new void RegisterEvents()
  {
    ...
    if (DefenseGrid.Active)
    {
      ...
      this.AddEvent(VehicleEventDefOf.Spawned,
        () => FrameDelay.DelayOne(static component =>
          {
            if (component is null) return;
