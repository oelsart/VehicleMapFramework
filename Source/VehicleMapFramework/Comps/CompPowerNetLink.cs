...
public abstract class CompPowerNetLink : CompPowerTrader
{
  ...
  public virtual int UpdateRateIntervalTicks => Connected ? 60 : 180;
  ...
  public override void CompTick()
  {
    ...
    var offset = parent.thingIDNumber < LinkedTo?.thingIDNumber ? UpdateRateIntervalTicks / 2 : 0;
    if (!GenTicks.IsTickInterval(offset, UpdateRateIntervalTicks)) return;
        ...
        var num = voltage > needs // バッテリーを使わず供給可
          ? Mathf.Min(diff > 0f
              ...
              : hasBattery
                ? needs
                : voltage,
            me.MaxPowerPush) // 過剰供給分は彼我のバッテリー差によってどっちが使うか決める
          : Mathf.Min(needs, me.MaxPowerPush); // 不足分はよりバッテリー量を持ってる方が補う
        ...
        var num2 = voltage2 > needs2 // むこうがバッテリーを使わず供給可
          ? -Mathf.Min(diff > 0f
              ...
              : hasBattery2
                ? needs2
                : voltage2,
            other.MaxPowerPush) // 過剰供給分は彼我のバッテリー差によってどっちが使うか決める
          : -Mathf.Min(needs2, other.MaxPowerPush); // 不足分はよりバッテリー量を持ってる方が補う
