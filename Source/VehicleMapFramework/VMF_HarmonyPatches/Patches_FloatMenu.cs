...
public static class Patch_FloatMenuContext_Constructor
{
  ...
  {
    Pawn pawn;
    if (!__instance.IsMultiselect && (pawn = __instance.FirstSelectedPawn) is not null)
  ...
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
  {
    ...
    var m_ThingsUnderMouseOnVehicle =
      ((Func<Vector3, float, TargetingParameters, ITargetingSource, List<Thing>>)GenUIOnVehicle.ThingsUnderMouse)
      .Method;
...
public static class Patch_FloatMenuOptionProvider_ExtinguishFires_GetSingleOption_Delegate
{
  ...
  {
    return AccessTools.FindIncludingInnerTypes(typeof(FloatMenuOptionProvider_ExtinguishFires),
      t => { return t.GetDeclaredMethods().FirstOrDefault(m => m.Name.Contains("<GetSingleOption>")); });
  }
...
{
  public static bool Prefix(Vector3 clickPos, TargetingParameters clickParams, bool thingsOnly, ITargetingSource source,
...
public static class Patch_FloatMenuOptionProvider_Entity_GetOptionFor
{
  ...
  {
    yield return AccessTools.FindIncludingInnerTypes(typeof(FloatMenuOptionProvider_CaptureEntity),
      GetOptionsFor_MoveNext);
    yield return AccessTools.FindIncludingInnerTypes(typeof(FloatMenuOptionProvider_TransferEntity),
      GetOptionsFor_MoveNext);
    yield break;
  ...
  {
    return enumerable.Concat(VehiclePawnWithMapCache.AllVehiclesOn(Find.CurrentMap).SelectMany(v =>
      v.VehicleMap.listerBuildings.AllBuildingsColonistOfClass<Building_HoldingPlatform>()));
  }
}
...
public static class Patch_MultiPawnGotoController_OnGUI
{
  ...
  private static Vector3 ToVector3Offset(IntVec3 intVec, Pawn pawn)
  {
    if (pawn.TryGetTargetMap(out var map))
    {
      ...
      {
        return Ext_Math.RotatePoint(intVec.ToVector3(), intVec.ToVector3Shifted(), vehicle.FullRotation.AsAngle)
          .ToBaseMapCoord(vehicle);
      }
    }
...
public static class Patch_FloatMenuOptionProvider_DraftedMove_PawnGotoAction
{
    ...
    List<TraverseSpots> spotsQueue, LocalTargetInfo dest)
  {
    ...
    else
    {
      ...
      else if (!baseMap.IsPlayerHome && !baseMap.exitMapGrid.MapUsesExitGrid && pawn.Map == baseMap &&
               CellRect.WholeMap(baseMap).IsOnEdge(clickCell, 3) &&
               baseMap.Parent.GetComponent<FormCaravanComp>() != null &&
               MessagesRepeatAvoider.MessageShowAllowed("MessagePlayerTriedToLeaveMapViaExitGrid-" + baseMap.uniqueID,
                 60f))
      {
        Messages.Message(
...
{
  public static void Prefix(Pawn pawn, WorkGiverDef workGiver, LocalTargetInfo target, FloatMenuContext context,
  ...
  public static void Finalizer(Pawn pawn, WorkGiverDef workGiver, LocalTargetInfo target, FloatMenuContext context,
    VirtualTeleporter? __state, FloatMenuOption __result)
  {
    if (__state is null)
    {
      return;
    ...
    if ((!__result?.Disabled ?? false) && __result.action != null &&
        JobAcrossMapsUtility.NeedWrapGotoDestMapJob(workGiver.Worker as WorkGiver_Scanner) &&
        pawn.CanReach(target, PathEndMode.Touch, Danger.Deadly, false, false, TraverseMode.ByPawn, context.map,
          out var exitSpot, out var enterSpot, out var spotsQueue))
    {
      __result.action = (() => { JobAcrossMapsUtility.StartGotoDestMapJob(pawn, exitSpot, enterSpot, spotsQueue); }) +
                        __result.action;
    }
  }
}