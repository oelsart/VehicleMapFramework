...
public static class Patch_Rendering_DrawSelectionBracketsVehicles
{
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
  {
    ...
    return matcher.InsertAndAdvance(
        ...
        CodeInstruction.LoadLocal(l_vehicle_ind),
        CachedMethodInfo.m_FlipAngle.CallInstruction)
      ...
      .Set(OpCodes.Call, ((Delegate)VehicleMapUtility.BaseRotatedSize).Method)
...
public static class Patch_TargetingHelper_TargetMeetsRequirements2
{
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
  {
    return instructions.MethodReplacer(
      ...
      (CachedMethodInfo.m_GenSight_LineOfSightToEdges, CachedMethodInfo.m_GenSightOnVehicle_LineOfSightToEdges));
  }
}
  ...
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
  {
    ...
    return new CodeMatcher(instructions, generator).Reset()
      ...
      .MatchStartForward(
        ...
        CodeMatch.Calls(((Delegate)CoverUtility.CalculateCoverGiverSet).Method))
...
public static class Patch_VehicleTurret_RotationAligned
{
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
  {
    ...
    return instructions.Manipulator(c => c.OperandIs(f_rotationTargeted),
      c =>
      {
  ...
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
  {
    foreach (var instruction in instructions)
    {
      ...
      if (instruction.opcode == OpCodes.Ldfld && ((FieldInfo)instruction.operand).FieldType == typeof(MapParent))
        yield return ((Delegate)Patch_LaunchProtocol_GetArrivalOptions.VehicleMapParentOrMe).Method.CallInstruction;
    }
  }
}
...
public static class Patch_LandingTargeter_GetPosState
{
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
  {
    ...
    foreach (var instruction in instructions)
    {
      ...
      {
        yield return ((Delegate)FocusedMapOrCurrentMap).Method.CallInstruction;
      }
    }
  }
      ...
      else
      {
        ...
        if (!exitMapOnArrival && !baseMap.IsPlayerHome && !baseMap.exitMapGrid.MapUsesExitGrid &&
            ...
            MessagesRepeatAvoider.MessageShowAllowed(
              $"MessagePlayerTriedToLeaveMapViaExitGrid-{baseMap.uniqueID}",
              60f))
        {
          string text = formCaravanComp.CanFormOrReformCaravanNow ? "MessagePlayerTriedToLeaveMapViaExitGrid_CanReform".Translate() : "MessagePlayerTriedToLeaveMapViaExitGrid_CantReform".Translate();
...
public static class Patch_VehicleOrientationController_TargeterUpdate
{
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
  {
    ...
    var m_ToVector3ShiftedOffsetWithAltitude = ((Delegate)Patch_MultiPawnGotoController_Draw.ToVector3ShiftedOffsetWithAltitude).Method;
    var num = 0;
...
public static class Patch_VehicleCaravan_Notify_MemberDied_Predicate
{
  ...
  {
    return AccessTools.FindIncludingInnerTypes(typeof(VehicleCaravan), t => { return t.GetDeclaredMethods().FirstOrDefault(m => m.Name.Contains("<Notify_MemberDied>")); });
  }
