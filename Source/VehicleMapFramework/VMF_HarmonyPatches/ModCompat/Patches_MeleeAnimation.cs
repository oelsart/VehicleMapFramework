  ...
  public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
  {
    ...
    return instructions.Manipulator(c => c.LoadsField(f_AnimRenderer_Map),
      c =>
      {
        ...
        c.operand = m_BaseMap;
      }).Manipulator(c => c.LoadsField(f_RootTransform),
      c =>
      {
        c.opcode = OpCodes.Call;
...
public static class Patch_PartWithSweep_Draw
{
  ...
  {
    var f_RootTransform = AccessTools.Field("AM.AnimRenderer:RootTransform");
    return instructions.Manipulator(c => c.opcode == OpCodes.Ldfld && c.OperandIs(f_RootTransform),
      c =>
      {
        c.opcode = OpCodes.Call;
...
public static class Patch_AnimRenderer_DrawSingle
{
  ...
  {
    return f_RootPositionOffset(instance);
  }
