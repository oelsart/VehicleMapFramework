...
public static class Patch_CompBuildingExtraRenderer_PostPrintOnto
{
  ...
  {
    return AccessTools.FindIncludingInnerTypes(GenTypes.GetTypeInAnyAssembly("Exosuit.CompBuildingExtraRenderer", "Exosuit"), t => { return t.GetDeclaredMethods().FirstOrDefault(m => m.Name.Contains("<PostPrintOnto>")); });
  }
...
public static class Patch_WG_PawnFlyer_ToDiffMapTarget
{
  ...
  public static void Prefix(ref Thing ___eBay, Pawn pawn)
  {
    if (___eBay == null)
    {
      var maps = pawn.MapHeld.BaseMapAndVehicleMaps(false);
