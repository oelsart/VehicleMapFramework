...
public static class HaulToInventoryAcrossMapsPatch
{
    ...
    public static MethodBase TargetMethod()
    {
        ...
        MethodInfo targetMethod = AccessTools.Method(workGiverType, "JobOnThing", [typeof(Pawn), typeof(Thing), typeof(bool)]);
        if (targetMethod == null)
        {
        ...
        if (pawn.Map == thing.Map)
        {
            ...
            if (vanillaHaulToInventoryMethod != null)
            {
                try
                {
                    ...
                    __result = (Job)vanillaHaulToInventoryMethod.Invoke(vanillaWorkGiver, [pawn, thing, forced]);
                    return false; // Skip the original method
                }
                catch (Exception ex)
                {
            ...
            try
            {
                ...
                if (StoreUtility.TryFindBestBetterStoreCellFor(thing, pawn, pawn.Map, currentPriority, pawn.Faction, out IntVec3 storeCell))
                {
