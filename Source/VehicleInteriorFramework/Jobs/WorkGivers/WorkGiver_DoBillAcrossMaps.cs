...
namespace VehicleInteriors
{
    ...
    public class WorkGiver_DoBillAcrossMaps : WorkGiver_DoBill, IWorkGiverAcrossMaps
    {
        ...
        {
            bool validator(Thing t) => !t.IsForbidden(pawn) && ((UnfinishedThing)t).Recipe == bill.recipe && (ModCompat.NoJobAuthors || ((UnfinishedThing)t).Creator == pawn) && ((UnfinishedThing)t).ingredients.TrueForAll((Thing x) => bill.IsFixedOrAllowedIngredient(x.def)) && pawn.CanReserve(t, t.Map);
        ...
        {
            if (uft.Creator != pawn && !ModCompat.NoJobAuthors)
            {
            ...
            {
                Job job2 = JobMaker.MakeJob(JobDefOf.DoBill, giver);
                job2.bill = bill;
        ...
        private Job StartOrResumeBillJob(Pawn pawn, IBillGiver giver, bool forced = false)
        {
            ...
            for (int i = 0; i < giver.BillStack.Count; i++)
            {
                ...
                if (bill is Bill_ProductionWithUft bill_ProductionWithUft)
                {
                    ...
                    {
                        if ((bill_ProductionWithUft.BoundWorker == pawn || ModCompat.NoJobAuthors) && bill_ProductionWithUft.BoundUft != null && pawn.CanReserveAndReach(bill_ProductionWithUft.BoundUft.Map, bill_ProductionWithUft.BoundUft, PathEndMode.Touch, Danger.Deadly, 1, -1, null, false, out var exitSpot, out var enterSpot) && !bill_ProductionWithUft.BoundUft.IsForbidden(pawn))
                        {
