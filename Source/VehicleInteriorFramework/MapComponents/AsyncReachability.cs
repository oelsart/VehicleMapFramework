...
namespace VehicleInteriors
{
    ...
    public class AsyncReachability : MapComponent
    {
        public AsyncReachability(Map map) : base(map)
        {
            ...
            {
                freeWorkers.Enqueue(new Worker(map, cache));
            }
        }
        ...
        {
            if (!freeWorkers.TryDequeue(out var worker))
            {
                Log.Error("[VehicleMapFramework] There is no available worker in AsyncReachability.");
            }
            try
            {
                return worker.CanReach(start, dest, peMode, traverseParams);
            }
            finally
            {
        ...
        private AsyncReachabilityCache cache = new AsyncReachabilityCache();
        ...
        public class Worker
        {
            ...
            private List<Region> closeList = new List<Region>();
            ...
            private AsyncReachabilityCache cache;
            ...
            private void DetermineStartRegions(IntVec3 start)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if (intVec.InBounds(map) && pathGrid.WalkableFast(intVec))
                    {
                        ...
                        if (validRegionAt2 != null && !closeList.Contains(validRegionAt2))
                        {
            ...
            private bool CheckCellBasedReachability(IntVec3 start, LocalTargetInfo dest, PathEndMode peMode, TraverseParms traverseParams)
            {
                ...
                if (foundCell.IsValid)
                {
                    if (CanUseCache(traverseParams.mode))
                    {
                        ...
                        if (validRegionAt != null)
                        {
                            for (int i = 0; i < startingRegions.Count; i++)
                            {
                                cache.AddCachedResult(startingRegions[i].District, validRegionAt.District, traverseParams, reachable: true);
                            }
                        }
                    }
                ...
                if (CanUseCache(traverseParams.mode))
                {
                    for (int j = 0; j < startingRegions.Count; j++)
                    {
                        ...
                        {
                            cache.AddCachedResult(startingRegions[j].District, destRegions[k].District, traverseParams, reachable: false);
                        }
                    }
                }
