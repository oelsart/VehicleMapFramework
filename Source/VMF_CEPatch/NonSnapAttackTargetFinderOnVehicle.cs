...
public class NonSnapAttackTargetFinderOnVehicle
{
  ...
  public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags, Vector3 angle, Predicate<Thing> validator = null, float minDist = 0f, float maxDist = 9999f)
  {
    ...
    else
    {
      ...
      result = (IAttackTarget)GenClosestCrossMap.ClosestThing_Global(validator: !num2 || flag2 ? t => innerValidator((IAttackTarget)t) : t => innerValidator((IAttackTarget)t) && CanShootAtFromCurrentPosition((IAttackTarget)t, searcher, verb),
        centerOnBaseMap: searcherThing.PositionOnBaseMapSpawned,
        searchSet: tmpTargets,
        maxDistance: maxDist);
    }
    tmpTargets.Clear();
    return result;
